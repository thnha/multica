...
description: Connect a self-hosted Forgejo, Gitea, or GitLab instance per workspace so pull/merge requests that carry an issue identifier in their branch name or title, or after a closing keyword in the body, auto-attach to that issue, move it to Done when they merge with a closing keyword, and show CI status.
---
...
- **Issue links** — an identifier in the title or branch links the PR to the issue, as does an identifier after a closing keyword in the body; a bare mention in the body links nothing. Once every PR linked to the issue is merged — across GitHub and self-hosted providers — and one of them used a closing keyword (`Closes`/`Fixes`/`Resolves MUL-123`), the issue moves to **Done**, under the same workspace setting and rules as [GitHub](/github-integration).
