...
リストの下には、次に何が起きるかを 1 行で表示します。たとえば「#19 がマージされると自動完了」、`Closes MUL-123` と書いた PR がないこと、マージされずにクローズされた PR です。

項目をクリックすると GitHub の PR が開きます。**PR サイドバー**を無効にしてもこのセクションが非表示になるだけで、接続は解除されません。

## PR のマージで Done になる条件

PR がマージされても、必ずしもタスクが完了するわけではありません。**PR のマージでタスクを自動完了**（**設定 → タスクステータス → 自動化**、既定でオン）が有効な場合、Multica がタスクを `Done` に変更するのは、次の条件をすべて満たす場合だけです。

1. 紐づいたマージ済み PR のうち少なくとも 1 つが、タイトルか本文で `Closes MUL-123` のように closing keyword の直後にタスク番号を指定している。`Closes login MUL-123` のように間に別の単語がある形式や、ブランチ名は対象外です。
2. タスクに紐づく PR がすべて `Merged` である。`Open` や `Draft` の PR が残っているとタスクは待機を続けます。マージされずにクローズされた PR も、タスクから外すまで同様です。
3. タスクが `done` または `cancelled` ではなく、そのタスクだけ自動完了をオフにしていない（**Pull requests → ⋯ → このタスクは自動完了しない**）。

そのため、ブランチ名やタイトルに `MUL-123` を含めるだけで紐づけは作成されますが、それだけではタスクを完了にしません。マージでタスクを完了させたい場合にだけ closing keyword を使ってください。closing keyword はマージ時点の内容で判断されます。マージ後に書き足しても紐づけが作成されるだけで、タスクは完了しません。
...
- **PR のマージ後もタスクが完了しない**: タスクの **Pull requests** リストの下にある説明を確認します。closing keyword を使った PR がないこと、まだマージされていない PR やマージされずにクローズされた PR、またはタスクやワークスペースで自動完了がオフになっていることが表示されます。
