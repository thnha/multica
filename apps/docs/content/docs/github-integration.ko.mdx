...
목록 아래에는 다음에 일어날 일을 한 줄로 보여 줍니다. 예를 들어 "#19 병합 시 자동 완료", `Closes MUL-123`을 쓴 PR이 없다는 안내, 병합되지 않고 닫힌 PR입니다.

항목을 클릭하면 GitHub의 PR이 열립니다. **PR 사이드바**를 꺼도 이 영역만 숨겨질 뿐 연결은 해제되지 않습니다.

## PR merge 시 Done 전환 조건

PR이 merge되었다고 해서 태스크가 반드시 끝난 것은 아닙니다. **PR 병합 시 태스크 자동 완료**(**설정 → 태스크 상태 → 자동화**, 기본값 켜짐)가 켜져 있을 때, Multica는 다음 조건을 모두 만족할 때만 태스크를 `Done`으로 바꿉니다.

1. merge된 연결 PR 중 적어도 하나가 제목이나 본문에서 `Closes MUL-123`처럼 closing keyword 바로 뒤에 태스크 번호를 썼습니다. `Closes login MUL-123`처럼 중간에 다른 단어가 있거나 브랜치 이름에만 있는 경우는 해당하지 않습니다.
2. 태스크에 연결된 PR이 모두 `Merged` 상태입니다. `Open`이나 `Draft` PR이 남아 있으면 태스크는 계속 기다립니다. merge되지 않고 닫힌 PR도 태스크에서 제거할 때까지 마찬가지입니다.
3. 태스크가 `done` 또는 `cancelled`가 아니고, 이 태스크만 자동 완료를 끄지 않았습니다(**Pull requests → ⋯ → 이 태스크는 자동 완료하지 않음**).

따라서 브랜치 이름이나 제목에만 `MUL-123`을 쓰면 연결은 되지만 그것만으로 태스크가 완료되지는 않습니다. merge와 함께 태스크를 끝내고 싶을 때만 closing keyword를 사용하세요. closing keyword는 merge 시점 기준으로 판단합니다. merge 후에 추가하면 연결만 되고 태스크는 완료되지 않습니다.
...
- **PR merge 후 태스크가 완료되지 않음**: 태스크의 **Pull requests** 목록 아래 설명을 확인합니다. closing keyword를 쓴 PR이 없는지, 아직 merge되지 않았거나 merge 없이 닫힌 PR이 있는지, 또는 태스크나 워크스페이스에서 자동 완료가 꺼져 있는지 알려 줍니다.
