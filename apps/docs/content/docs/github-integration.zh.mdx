...
列表下方有一行说明接下来会发生什么，例如「等待 #19 合并后自动完成」、没有 PR 写 `Closes MUL-123`，或者哪个 PR 已关闭、未合并。

点击记录可以打开 GitHub 上的 PR。关闭 **PR 侧栏** 只会隐藏这个区块，不会断开连接。

## PR 合并转 Done 的条件

PR 合并不一定代表任务已完成。开启 **PR 合并后自动完成任务**（默认开启）时，只有以下条件同时成立，任务才会改为 `Done`：

1. 至少一个已合并的关联 PR 在标题或正文中使用了紧跟编号的关闭语句，例如 `Closes MUL-123`（`Closes login MUL-123` 这类中间隔词的写法不生效，分支名也不算）；
2. 所有关联 PR 都是 `Merged`。仍为 `Open` 或 `Draft` 的 PR 会让任务继续等待；关闭但未合并的 PR 也一样，直到你把它从任务上移除；
3. 任务当前不是 `done` 或 `cancelled`，也没有单独关闭自动完成（**Pull requests → ⋯ → 此任务不自动完成**）。

因此，仅在分支名或标题中写 `MUL-123` 会建立关联，但不会单独触发完成；只有希望 PR 合并即完成任务时，才使用关闭语句。关闭语句以合并时为准：PR 合并后再补写，只会建立关联，不会完成任务。
...
- **PR 合并后任务没完成**：看任务 **Pull requests** 列表下方的说明，它会指出没有 PR 使用关闭语句、仍未合并或已关闭未合并的 PR，或者说明任务、工作区关闭了自动完成。
